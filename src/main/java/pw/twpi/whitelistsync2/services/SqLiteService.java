...
public class SqLiteService implements BaseService {
    ...
    public boolean initializeDatabase() {
        ...
        File databaseFile = new File(ModConfig.SQLITE_DATABASE_PATH.get());
        ...
        if (!databaseFile.exists() && isSuccess) {
            String url = "jdbc:sqlite:" + ModConfig.SQLITE_DATABASE_PATH.get();
            try {
                Connection conn = DriverManager.getConnection(url);

                WhitelistSync2.LOGGER.info("A new database \"" + ModConfig.SQLITE_DATABASE_PATH.get() + "\" has been created.");
                conn.close();
        ...
        if (isSuccess) {
            try {
                Connection conn = DriverManager.getConnection("jdbc:sqlite:" + ModConfig.SQLITE_DATABASE_PATH.get());
