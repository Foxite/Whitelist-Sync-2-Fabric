...
public class CommandSync implements Command<ServerCommandSource> {
    ...
    private static final SimpleCommandExceptionType DB_ERROR
            = new SimpleCommandExceptionType(new LiteralText("Error syncing op database, please check console for details."));
    ...
    public int run(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
        if(WhitelistSync2.whitelistService.copyDatabaseOppedPlayersToLocal(context.getSource().getMinecraftServer())) {
            context.getSource().sendFeedback(new LiteralText("Local op list up to date with database."), false);
        } else {
