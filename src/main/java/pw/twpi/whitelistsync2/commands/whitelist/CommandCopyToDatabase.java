...
public class CommandCopyToDatabase implements Command<ServerCommandSource> {
    ...
    private static final SimpleCommandExceptionType DB_ERROR
            = new SimpleCommandExceptionType(new LiteralText("Error syncing local whitelist to database, please check console for details."));
    ...
    public int run(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
        ...
        if(WhitelistSync2.whitelistService.copyLocalWhitelistedPlayersToDatabase()) {
            context.getSource().sendFeedback(new LiteralText("Pushed local whitelist to database."), false);
        } else {
